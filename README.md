...
メーカーごとの機種特有の情報（型式の読み方、特定機種のエラーコード表、
専用ツールの使い方など）を、将来的に追加していく想定がある。
その際は以下のような構成を検討する：

- 既存教材にメーカー別のアコーディオン項目を追記する（軽量な追加向き）
- `category: "メーカー別技術資料"` を持つ新しいモジュールを追加し、
  メーカーごとにサブページを作る（本格的に増える場合向き）

いずれの場合も `modules.json` にエントリを追記するだけでポータルに反映される
構成になっているため、追加方針が決まり次第スムーズに拡張できる。
